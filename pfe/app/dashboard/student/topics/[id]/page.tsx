  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {hasTopic && (
          <span className="px-6 py-3 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-sm font-semibold">
            Sujet déjà assigné
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="relative bg-white rounded-2xl border border-gray-200 p-8 shadow-2xl">
            ...
            <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">{topic.description || 'Aucune description'}</p>
          </div>

          {topic.requirements && (
            <div className="relative bg-white rounded-2xl border border-gray-200 p-8 shadow-2xl">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Prérequis et compétences</h2>
              <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">{topic.requirements}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              {topic.department && (
                <div>
                  ...
                  <p className="text-gray-900 font-medium capitalize">{topic.department}</p>
                </div>
              )}
              {professor && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Proposé par</p>
                  <p className="text-gray-900 font-medium">{professor.full_name}</p>
                  {professor.email && (
                    <a
          ...
          {professor && (professor.office || professor.office_hours || professor.bio) && (
            <div className="relative bg-white rounded-2xl border border-gray-200 p-6 shadow-xl">
              <h3 className="text-lg font-bold text-gray-900 mb-4">À propos de l&apos;auteur du sujet</h3>
              <div className="space-y-4">
                {professor.office && (
                  <div>
                ...
                {professor.office_hours && (
                  <div>
                    ...
                    <p className="text-gray-900 font-medium text-sm">{professor.office_hours}</p>
                  </div>
                )}
                {professor.bio && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Biographie</p>
                    <p className="text-gray-700 text-sm leading-relaxed">{professor.bio}</p>
                  </div>
                )}
