async function getEvents() {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/professor/calendar`, {
      cache: 'no-store',
    })
    if (!res.ok) return []
          ...
          <div className="relative bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-8 shadow-2xl">
            ...
            <div className="space-y-4">
              {events && events.length > 0 ? events.map((event: any) => {
                ...
                const timeStr = event.time ? `${event.time}` : eventDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${typeColors[eventType] || typeColors.meeting} flex flex-col items-center justify-center border shrink-0`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="text-white font-semibold text-lg">{event.title || 'Événement'}</h3>
                          <span className={`px-3 py-1 rounded-lg text-xs font-semibold bg-gradient-to-r ${typeColors[eventType] || typeColors.meeting}`}>
                            {eventType === 'meeting' && 'Réunion'}
                            {eventType === 'review' && 'Révision'}
                            {eventType === 'presentation' && 'Présentation'}
                            {!['meeting', 'review', 'presentation'].includes(eventType) && 'Événement'}
                          </span>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-gray-400">
                          ...
                          {event.student && (
                            <span className="flex items-center gap-1">
                              ...
                              {event.student.full_name || event.student || 'N/A'}
                            </span>
                          )}
                        </div>
                        {event.description && (
